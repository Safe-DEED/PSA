...
async function getServerContext({
  ...
  return await (0, _HEutil.createServerContext)(polyModulusDegree, plainModulusBitSize, securityLevel, compressionMode, maskHW, minHW, maskBin, createGkIndices, diffPriv, sensitivity, epsilon);
}
/**
...
function clientEncrypt(inputArray, clientContext, serialize = true) {
  ...
  return (0, _apiUtil.getClientRequestObject)(encryptedArray, hw, clientContext, serialize);
}
/**
...
function clientDecrypt(serverResponseObject, clientContext) {
  const encryptedResult = typeof serverResponseObject === 'string' ? parse(serverResponseObject) : serverResponseObject;
...
async function serverCompute(clientRequestObject, matrix, serverContext, serialize = true) {
  ...
  const {
    ...
    galois,
    relin
  } = typeof clientRequestObject === 'string' ? parse(clientRequestObject) : clientRequestObject;
  ...
  const computationResult = await (0, _apiUtil.compute)(arrayOfBase64EncodedCiphertexts, hw, matrix, serverContext);
  return (0, _apiUtil.getServerResponseObject)(computationResult, serialize);
}
/**
