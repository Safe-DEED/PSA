 ...
 */
export function laplace(n, loc, scale) {
  n = _v(n, 'n');
  loc = _v(loc, 'r', 0);
  scale = _v(scale, 'nn', 1);
...
function sample(collection, n, replace, ratios) {
  ...
  let cumulativeProbs = _getCumulativeProbs(ratios, collection.length);
...
function _getCumulativeProbs(ratios, len) {
  if (len === undefined)
    throw new Error(
      'An error occurred: len was not sent to _getCumulativeProbs'
    );
  if (ratios.length !== len)
    throw new Error(
...
function _v(param, type, defaultParam) {
  ...
  switch (type) {
    ...
    case 'n':
      ...
      if (param < 1)
        throw new Error(
          'The number of values must be a whole number of 1 or greater'
        );
      if (param === Infinity)
